...
#define WWW_PATH "/afs/cern.ch/user/l/lforthom/www/private/dm/ee/tmp/"
//#define WWW_PATH "/afs/cern.ch/user/l/lforthom/www/private/dm/ee/highxicut/"
...
void analyze_ee()
{
  ...
  //const vector<double> miss_mass_bins = { 200., 400., 600., 800., 1000., 1200., 1400., 1600., 1800., 2000., 2400., 2600. };
  ...
  vector<TH1D>
    h_m_pair_all( 3, TH1D( "h_m_pair_all", "m(e^{+}e^{-})\\Events\\GeV", 70, 50., 400. ) ),
    h_pt_pair_all( 3, TH1D( "h_pt_pair_all", "p_{T}(e^{+}e^{-})\\Events\\GeV", 50, 25., 275. ) ),
    h_pp_mass_all( 3, TH1D( "h_pp_mass_all", "m_{pp}\\Events\\GeV", 60, 100., 2500. ) ),
    h_rap_pair_all( 3, TH1D( "h_rap_pair_all", "Y(e^{+}e^{-})\\Events", 20, -3., 3. ) ),
    h_acop_all( 3, TH1D( "h_acop_all", "1-|#Delta#phi(e^{+}e^{-})/#pi|\\Events", 50, 0., 1. ) ),
    h_xangle_all( 4, TH1D( "h_xangle_all", "Crossing angle\\Events\\#murad", 50, 100., 200. ) ),
    //-----
    h_m_pair( 2, TH1D( "h_m_pair", "m(e^{+}e^{-})\\Events\\GeV", 70, 50., 400. ) ),
    h_rap_pair( 2, TH1D( "h_rap_pair", "Y(e^{+}e^{-})\\Events", 20, -3., 3. ) ),
    h_pt_pair( 2, TH1D( "h_pt_pair", "p_{T}(e^{+}e^{-})\\Events\\GeV", 50, 25., 275. ) ),
    h_pt_leadele( 2, TH1D( "h_pt_leadele", "Leading e^{#pm} p_{T}\\Events\\GeV", 50, 0., 250. ) ),
    h_pt_subleadele( 2, TH1D( "h_pt_subleadele", "Subleading e^{#pm} p_{T}\\Events\\GeV", 50, 0., 250. ) ),
    h_eta_leadele( 2, TH1D( "h_eta_leadele", "Leading e^{#pm} #eta\\Events\\?g", 50, -2.5, 2.5 ) ),
    h_eta_subleadele( 2, TH1D( "h_eta_subleadele", "Subleading e^{#pm} #eta\\Events\\?g", 50, -2.5, 2.5 ) ),
    h_acop( 2, TH1D( "h_acop", "1-|#Delta#phi(e^{+}e^{-})/#pi|\\Events", 50, 0., 1. ) ),
  ...
  ch.Add( "/eos/cms/store/group/phys_pps/dilepton/DoubleEG/ee_ntuple-Run2017B-ptgt50_v1.root" ); runs_list.emplace_back( "B" ); int_lumi += 2.367133065086;
  ch.Add( "/eos/cms/store/group/phys_pps/dilepton/DoubleEG/ee_ntuple-Run2017C-ptgt50_v1.root" ); runs_list.emplace_back( "C" ); int_lumi += 8.680670867306;
  ch.Add( "/eos/cms/store/group/phys_pps/dilepton/DoubleEG/ee_ntuple-Run2017D-ptgt50_v1.root" ); runs_list.emplace_back( "D" ); int_lumi += 4.142654224602;
  ch.Add( "/eos/cms/store/group/phys_pps/dilepton/DoubleEG/ee_ntuple-Run2017E-ptgt50_v1.root" ); runs_list.emplace_back( "E" ); int_lumi += 9.061579665808;
  ch.Add( "/eos/cms/store/group/phys_pps/dilepton/DoubleEG/ee_ntuple-Run2017F-ptgt50_v1_partial.root" ); runs_list.emplace_back( "F" ); int_lumi += 13.920866597190;

  PaveText pt_runs( 0.82, 0.64, 0.85, 0.73 );
  pt_runs.AddText( "e^{+}e^{-} channel" );
  ...
  while ( ( current_file = (TFile*)fileIter.Next() ) ) {
    ...
    tree->SetBranchStatus( "EleCand_et", 1 ); tree->SetBranchStatus( "EleCand_eta", 1 ); tree->SetBranchStatus( "EleCand_phi", 1 ); tree->SetBranchStatus( "EleCand_e", 1 );
    tree->SetBranchStatus( "EleCand_charge", 1 ); tree->SetBranchStatus( "EleCand_tightID", 1 );
    //tree->SetBranchStatus( "", 1 );
    ggll::AnalysisEvent evt;
    evt.load( tree, ggll::DiElectron, false );
...
//          if ( xi > 0.25 ) continue;
      ...
      for ( unsigned int j = 0; j < evt.nPair; ++j ) {
        ...
        if ( evt.EleCand_et[l1] < 35. || evt.EleCand_et[l2] < 35. ) continue;
        if ( fabs( evt.EleCand_eta[l1] ) > 2.5 || fabs( evt.EleCand_eta[l2] ) > 2.5 ) continue;
        if ( evt.EleCand_charge[l1]*evt.EleCand_charge[l2] >= 0 ) continue;
        if ( evt.EleCand_tightID[l1] == 0 || evt.EleCand_tightID[l2] == 0 ) continue;
        ...
        pl1.SetPtEtaPhiE( evt.EleCand_et[l1], evt.EleCand_eta[l1], evt.EleCand_phi[l1], evt.EleCand_e[l1] );
        pl2.SetPtEtaPhiE( evt.EleCand_et[l2], evt.EleCand_eta[l2], evt.EleCand_phi[l2], evt.EleCand_e[l2] );
      ...
      h_pt_leadele[r].Fill( pl_lead.Pt() );
      h_pt_subleadele[r].Fill( pl_sublead.Pt() );
      h_eta_leadele[r].Fill( pl_lead.Eta() );
      h_eta_subleadele[r].Fill( pl_sublead.Eta() );
  ...
  unordered_map<string,vector<TH1D>&> plots_1d = {
    ...
    { "cand_ptlead", h_pt_leadele }, { "cand_ptsublead", h_pt_subleadele },
    { "cand_etalead", h_eta_leadele }, { "cand_etasublead", h_eta_subleadele },
